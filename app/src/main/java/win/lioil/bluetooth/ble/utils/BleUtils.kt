...
class BleUtils private constructor(){
    ...
    fun read(){
        ...
        if (service != null) {
            ...
            val characteristic = service.getCharacteristic(Constant.UUID_CHAR_READ_NOTIFY)
    ...
    fun write(content : String){
        ...
        if (service != null) {
            ...
            val characteristic = service.getCharacteristic(Constant.UUID_CHAR_WRITE)
            //单次最多20个字节
    ...
    fun setNotify(){
        ...
        if (service != null) {
            ...
            val descriptor = characteristic.getDescriptor(Constant.UUID_DESC_NOTITY)
    ...
    fun bleServiceSetting(){
        val bluetoothManager = MyApplication.instance?.getSystemService(Activity.BLUETOOTH_SERVICE) as BluetoothManager
        ...
        val scanResponse = AdvertiseData.Builder()
            ...
            .addServiceUuid(ParcelUuid(Constant.UUID_SERVICE)) //服务UUID
        ...
        // =============启动BLE蓝牙服务端=====================================================================================
        val service = BluetoothGattService(Constant.UUID_SERVICE, BluetoothGattService.SERVICE_TYPE_PRIMARY)
        //添加可读+通知characteristic
        val characteristicRead = BluetoothGattCharacteristic(
            Constant.UUID_CHAR_READ_NOTIFY,
            BluetoothGattCharacteristic.PROPERTY_READ or BluetoothGattCharacteristic.PROPERTY_NOTIFY,
            BluetoothGattCharacteristic.PERMISSION_READ
        )
        characteristicRead.addDescriptor(
            BluetoothGattDescriptor(Constant.UUID_DESC_NOTITY, BluetoothGattCharacteristic.PERMISSION_WRITE)
        )
        service.addCharacteristic(characteristicRead)
        //添加可写characteristic
        val characteristicWrite = BluetoothGattCharacteristic(
            Constant.UUID_CHAR_WRITE,
            BluetoothGattCharacteristic.PROPERTY_WRITE, BluetoothGattCharacteristic.PERMISSION_WRITE
        )
        service.addCharacteristic(characteristicWrite)
        if (bluetoothManager != null) {
            mBluetoothGattServer = bluetoothManager.openGattServer(MyApplication.instance, GattCallbackUtils.instance.mBluetoothGattServerCallback)
        }
